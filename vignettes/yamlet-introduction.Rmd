...
data items are identified.  The most
direct way is to supply explicit `yaml` keys:
...
An early use of metadata is to guide table development
in a collaborative setting (i.e. as a data specification).
In contrast with this prospective use is the retrospective
use of metadata for reporting.  Curated metadata, 
appropriately associated with primary data, can 
enrich output such as tables and figures consistently.  
...
below, we read attributes twice: once to get 'guide'
as 'guide' so that `coerce` will factorize 'Heart',
and again to get 'guide' as 'units' (technically wrong
for categorical items) which is what `table1()` needs.
